...
class CadAdaptiveExtendedObservation(BaseModel):
    ...
    supersedes_observation_sha256: str | None = Field(default=None, pattern=r'^[0-9a-f]{64}    created_at_utc: str = Field(min_length=1)
...
def build_adaptive_extended_plan(
    *,
    ...
    observation_sha256: str = Field(pattern=r'^[0-9a-f]{64}    created_at_utc: str = Field(min_length=1)
...
def build_adaptive_extended_plan(
    *,
    ...
    for observation in observations:
        ...
        if key in by_key:
            raise ValueError(
                'extended adaptive evidence is ambiguous for candidate/objective: '
                f'{key[0]} / {key[1]}'
            )
        by_key[key] = observation
    objective_ids = tuple(sorted({item.objective_id for item in observations}))
