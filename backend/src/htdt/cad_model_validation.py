...
class CadModelValidationRecord(BaseModel):
    ...
    search_spec_sha256: str = Field(pattern=r'^[0-9a-f]{64}    requested_band_hz: tuple[float, float]
    ...
    def valid_identity(self):
        ...
        if len({(pair.prediction_source_id, pair.measurement_id) for pair in self.pairs}) != len(self.pairs):
        ...
        if self.holdout_rms_db is not None and self.holdout_rms_db > self.max_holdout_rms_db and self.recommendation_gate == 'eligible':
            raise ValueError('recommendation cannot be eligible when holdout RMS exceeds the configured gate')
    ...
    def identity_payload(self):
        return {
            'document_id': self.document_id, 'search_spec_id': self.search_spec_id,
            'search_spec_sha256': self.search_spec_sha256, 'candidate_set_sha256': self.candidate_set_sha256,
...
def build_model_validation(*, document_id: str, search_spec_id: str, search_spec_sha256: str,
    candidate_set_sha256: str, model_id: str, model_version: str,
    ...
    payload={'document_id':document_id,'search_spec_id':search_spec_id,
        'search_spec_sha256':search_spec_sha256,'candidate_set_sha256':candidate_set_sha256,
        'model_id':model_id,'model_version':model_version,'requested_band_hz':(low_hz,high_hz),
    ...
    candidate_set_sha256: str = Field(pattern=r'^[0-9a-f]{64}    requested_band_hz: tuple[float, float]
    pairs: tuple[CadValidationPair, ...] = Field(min_length=1)
    holdout_rms_db: float | None = Field(default=None, ge=0)
    calibration_rms_db: float | None = Field(default=None, ge=0)
    recommendation_gate: Literal['disabled', 'eligible']
