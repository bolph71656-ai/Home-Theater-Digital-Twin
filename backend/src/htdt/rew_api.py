...
def decode_rew_float_array(encoded: str) -> tuple[float, ...]:
    ...
    if not raw:
        return ()
    return tuple(struct.unpack(f'>{len(raw) // 4}f', raw))
...
def decode_frequency_response(measurement_id: str, payload: dict[str, Any]) -> RewFrequencyResponse:
    ...
    ppo = float(ppo_raw) if isinstance(ppo_raw, (int, float)) and ppo_raw > 0 else None
    step = float(step_raw) if isinstance(step_raw, (int, float)) and step_raw > 0 else None
    ...
    def get_frequency_response(
        ...
        *,
        ...
        payload = self._get_json(
            f'/measurements/{quote(measurement_id, safe="")}/frequency-response',
            {'ppo': ppo, 'unit': unit},
        )
        if not isinstance(payload, dict):
            raise RewApiError('Unexpected REW frequency-response response shape')
        return decode_frequency_response(measurement_id, payload)
