...
> 改訂: 2026-09-16 / 状態: v0.1実装進行中・長期計画更新
> 対象: Windows・個人利用・ローカル完結・サブウーファーなしを優先
> 未検証の技術選択は、実装済み・動作確認済みとは扱わない。配置探索は独立検証を通過した段階だけ有効化する。
...
| [配置探索ロードマップ](PLACEMENT_OPTIMIZATION_ROADMAP.md) | 候補生成、シミュレーション、多目的探索、実測閉ループ、適応探索 |
...
| v0.1基盤実装 | 進行中 | 実装済み/未検証はIMPLEMENTATION_STATUSを正本とし、長期探索機能を実装済み扱いしない |
...
- REWの演算結果・予測・出所不明も取込時に分類し、指定された.mdat・校正ファイル・設定メモを添付保存。
- 複数応答の重ね描き、FL/FR差、条件A/B差、保存した比較設定の再現。
- 測定品質と同条件の繰返し差を示し、条件訂正やアプリ更新後も保存済み比較を保持。
...
APIによる取込省力化、IR/ETC、モード・境界反射の候補表示、REW Room Simulator連携、3D反射経路、実測候補の配置比較を進める。配置探索は[配置探索ロードマップ](PLACEMENT_OPTIMIZATION_ROADMAP.md)を正式な正本とし、探索空間生成、バッチ予測、多目的評価、Pareto探索、実測閉ループ、独立検証、条件付き適応探索まで段階化する。
...
.mdatはREWで再解析するための任意添付として保管する。提供されたものはプロジェクトとバックアップへ含め、未提供なら不足を表示する。HTDTの内部標準形式にはせず、読込みはREWに任せる。
...
- 条件訂正・再出力も不変の版として保存し、比較はMeasurement版・Dataset版・Context版を固定参照する。旧算法を再実行できなくても保存済み結果を表示できるようにする。
...
- ブラウザーのタブを閉じる操作とアプリ終了を分ける。初期は起動用ターミナルのCtrl+Cで正常終了できる手順を用意し、次の起動でプロジェクトを開けることを確認する。
...
| 配置探索 O10–O40 | 探索空間、バッチ予測、多目的評価、Pareto探索 | モデル適用条件と探索結果を再現し、単一総合点へ潰さず候補を比較できる |
| 配置探索 O50–O60 | 実測閉ループ、保留配置によるモデル検証 | 候補→実測→残差を追跡し、不安定なモデルでは推薦を停止できる |
| 配置探索 O70+ | 条件付き適応探索、拡張変数 | 独立検証を通過した場合のみ次測定候補を提案できる |
| その他条件付き | 任意AVR読取、任意パッケージ化 | 各機能の検証条件を満たした場合のみ |

v1.0を「自動最適化が完成した状態」と定義しない。信頼できる比較・記録ツールとして完了できる計画にする。カレンダー上の納期は実データの準備と実装時間が不明なため約束しない。

v0.2/v0.5は追加機能のまとまりであり、v1.0への必須の直列工程ではない。v0.1の実利用で必要性が確認できた機能だけを選び、API・IR・3D・予測の導入待ちで比較管理の安定版を遅らせない。配置探索O10以降も同様に独立トラックとし、v1.0の安定化をブロックしない。
...
| 配置探索 | 可動範囲・制約を設定しPareto候補を比較 | モデル適用範囲、各目的値、移動量、予測/実測区分 |
| モデル検証 | 予測と実測の残差・保留配置を確認 | 学習/検証区分、適用帯域、感度、推薦可否 |
...
8. 配置探索は複数目的を保持し、単一総合点だけで「最適」と断定しない。
9. 適応探索は独立した実測検証を通過するまで次測定候補の提案に使わない。
