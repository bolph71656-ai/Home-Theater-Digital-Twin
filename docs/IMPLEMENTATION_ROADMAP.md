...
> 2026-09-15 / 以下は実装開始を指示された後の作業計画。
> 今回はアプリ、依存環境、テストコード、CI、Issue群を作成しない。

## 1. 進め方

基盤を全部作ってから使える画面を作るのではなく、「一つの実ファイルを取り込み、保存して、比較を再現する」縦断実装から始める。各作業単位は関連する文書・受入条件と一緒に短いブランチ/PRにする。手順のためだけの大量チケットや承認工程は設けない。

設計の正本は[全体計画](PROJECT_PLAN.md)、[測定・連携](MEASUREMENT_WORKFLOW.md)、[データ・解析](DATA_AND_ANALYSIS.md)。
...
| P0-03 | 校正・単位・平滑化・窓 | 出力設定のメモ/画像と原本 | 不明項目の表現を先に確認 |
...
| M02 | 実テキストの調査、初期parser、原本・単位契約 | P0-02/03、M01 | 元の行・周波数・値を保存でき、非対応形式を説明できる |
| M03 | Project/Revision/Context/MeasurementとSQLite | M02 | 取込・再起動・再読込ができ、途中失敗で測定を残さない |
| M04 | 周波数グラフと比較契約 | M03 | 2本の応答とA−Bを正しい単位で表示し比較設定を再現 |
| M05 | 部屋・スピーカー・測定点の数値編集と平面図 | M03、P0-04 | 配置版を複製し、旧測定の座標を保持 |
| M06 | 取込プレビューとチャンネル/条件の割当 | M04、M05 | ファイル名の推定を確認でき、unknownを保持 |
| M07 | 実際のA/B比較、条件差表示、履歴一覧 | M06 | 入力/音源を区別し、意図した変更と交絡要因を表示 |
| M08 | バックアップ・別フォルダ復元・スキーマ版 | M03、M07 | 元ファイルを移動しても復元先で同じ曲線と条件を表示 |
...
| X01 | 条件付き: 未測定位置の探索 | 応答モデルと独立検証 | 仮定の感度と実測確認手順を伴う候補を出せる |
| X02 | 条件付き: AVR状態読取 | 機種と公開仕様が確定 | 取得できる値と手入力値・不明を区別 |
| X03 | 条件付き: パッケージ化 | コマンド起動に具体的不便 | 対象Windowsで導入・終了・再起動・削除を確認 |

APIとIRは独立した追加機能なので、一方の機器・データ待ちで他方を止めない。高度なシミュレーションや探索が未達でもv1.0は成立する。
...
| backend/src/htdt/integrations | 後続のREW API/AVRアダプター |
...
| 予測探索 | 検証配置の傾向や順位が不安定 | 自動ランキングを提供せず実測候補を比較 |
