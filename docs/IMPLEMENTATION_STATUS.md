...
| 次工程 | **R100BはIssue #179でPFFDTD exact concave / spatial evidenceを進める。spatial incident/reflected decompositionはsolver-neutral authority未定義のためBLOCKEDを維持し、qualified independent concave referenceが得られるまではcandidate traceをreference truthへ昇格しない。その後geometric obstacle、candidate-wide hard gateとADR readinessへ進む。production solverは未選定、R110は引き続きblocked。O90はO90C以降を別sliceとして進め、O100はR110 interface確定までO100C source authorityを先走らない。Issue #83 owned-room gateも未完了** |
...
- PFFDTD remaining exact-concave/spatial wave evidence、solver-neutral spatial reflection decomposition、geometric obstacle、external benchmark/candidate-wide hard gates、production stack ADRは未完了。R100B完了とは扱わない。
- RDCは使用しない。
