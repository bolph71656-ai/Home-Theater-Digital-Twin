...
> 更新: 2026-09-19 / N05〜N90 + O10〜O80 software path実装済み / O90A + bounded O90B slice実装済み・canonical O90B残件あり / O100A–O100B実装済み・O100C以降未実装 / Issue #101 R100A-2 authority + R100B bakeoff実装中 / synthetic acceptance対象 / 実室model gate未通過
...
| 区分 | 現在の状態 |
|---|---|
| main | **N05〜N90 stable releaseとO10〜O80 software pathをmerge済み**。O90A + bounded O90B sliceはPR #145/#149で実装、canonical O90B残件とO90C以降は未実装。O100A–O100BはPR #144/#150で実装、O100C以降は未実装。O80 PR #94 merge `6faf554bcf3670f64ff13c530fa4fc79ab1881b8` / CI #548 PASS / Windows Release Artifact #93 PASS |
...
| O90 tracking | Issue #140 / #146。O90AをPR #145、bounded multidimensional sampling / linked axes / sampled envelope / feasible fraction / O40 robust ParetoをPR #149でmain反映済み。#146のdistribution/empirical/discrete + cancel/cache/resume/staleは未実装 |
...
| R100B tracking | PR #111 authority / #112 pyroom / #113 PFFDTD platform / #114 MFEM rigid reference / #115 PFFDTD rigid modes / #116 R100A-2 complex-pressure convergence / #151 explicit impedance native-boundary reflection gateまでmain反映済み。#151 fixtureはPASS。ただしspatial FDTD reflection validationとADRは未完了 |
| 次工程 | **R100BはMFEM concave/impedance independent reference、pyroom stochastic-seed/convergence、concave/portal/obstacle等の残gateへ進む。#151 native impedance PASSだけでproduction solverを選定しない。O90はIssue #146残件、O100はR110 interface確定までO100C source authorityを先走らない。Issue #83 owned-room gateも未完了** |
...
- MFEM concave/impedance independent reference、PFFDTD remaining spatial wave fixtures、pyroom stochastic convergence、portal/obstacle/external benchmark、production stack ADRは未完了。R100B完了とは扱わない。
- RDCは使用しない。
