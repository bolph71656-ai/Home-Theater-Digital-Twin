...
| R100B tracking | PR #111 authority / #112 pyroom direct+first-reflection / #113 PFFDTD platform / #114 MFEM rigid reference / #115 PFFDTD rigid modes / #116 R100A-2 complex-pressure convergence / #151 PFFDTD impedance / #154 MFEM concave independent reference / #155 pyroom stochastic evidence / #158 fail-closed low-band wave adoption profile / #160 MFEM Portal continuityまでmain反映済み。#160 Portal fixtureは281点で magnitude/phase/complex差0のPASS。MFEM concave FAIL・impedance extraction BLOCKEDは維持 |
| 次工程 | **R100BはIssue #161でexplicit radiation terminationの数式authorityを先に固定し、そのreference gateを実装する。続いてPFFDTD spatial concave/reflection、geometric obstacle、candidate-wide hard gateとADR readinessへ進む。Portal PASSだけでproduction solverを選定しない。O90はO90C以降を別sliceとして進め、O100はR110 interface確定までO100C source authorityを先走らない。Issue #83 owned-room gateも未完了** |
...
- PFFDTD remaining spatial wave fixtures、explicit radiation termination、geometric obstacle、external benchmark/candidate-wide hard gates、production stack ADRは未完了。R100B完了とは扱わない。
- RDCは使用しない。
