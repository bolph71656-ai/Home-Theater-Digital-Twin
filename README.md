...
v0.1の中核であるプロジェクト保存、配置/条件スナップショット、REWテキスト取込、A/B比較、バックアップ/復元、最小空間表示、Windows合成E2Eまで実装済みです。測定マイクと実REWデータはまだないため、実機互換性の最終受入だけは保留しています。
...
REW 5.40系のAPIについてはlocalhost限定・GET専用のブラウザを実装済みです。REW未起動時はoffline表示となり、接続時だけ測定一覧とFRをプレビューします。REWの測定開始、Generator、設定変更、API取得データの自動保存は行いません。
...
実REW出力がまだないため、parserとAPIアダプターは合成fixture/mockではテスト済みでも、所有PC上の特定REWビルドとの互換性をまだ保証しません。また、HTDTは現段階で「最適位置」を自動断定しません。まず同条件再測定のばらつきと配置A/B差を比較し、再現した改善を次の基準にします。
