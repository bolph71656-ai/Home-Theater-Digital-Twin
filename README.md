...
v0.1の中核であるプロジェクト保存、配置/条件スナップショット、REWテキスト取込、A/B比較、バックアップ/復元、空間表示に加え、矩形室モード・一次反射の幾何候補と、任意のREW read-only API接続を実装しています。測定マイクと実REWデータはまだないため、実機互換性の受入だけは保留しています。
...
7. 配置を変える場合は選択版を複製して新しいContext revisionを作り、過去測定を現在位置へ動かさない。
8. 必要に応じて矩形室モード/一次反射の**予測候補**を表示するが、実測原因とは断定しない。
9. DBとRawAssetをZIPへバックアップし、別データ領域へ復元できる。

## 任意のREW API

REW APIが起動している場合、HTDTは `http://127.0.0.1:4735` から測定一覧と周波数応答をGETだけで読み取れます。POST/PUT/DELETEは初期アダプターに実装していません。

```text
GET /api/rew/status
GET /api/rew/measurements
GET /api/rew/measurements/{uuid}/frequency-response
```

詳細は [REW_API.md](docs/REW_API.md) を参照してください。
...
| [REW_API.md](docs/REW_API.md) | localhost限定・GET専用REW APIアダプター |
...
実REW出力/API接続がまだないため、parser/API adapterは合成fixture・mockではテスト済みでも、所有PC上のREWとの互換性をまだ保証しません。また、HTDTは現段階で「最適位置」を自動断定しません。まず同条件再測定のばらつきと配置A/B差を比較し、再現した改善を次の基準にします。
